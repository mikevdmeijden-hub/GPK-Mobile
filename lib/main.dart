// ---------------------------------------------------------------------------
// GPP Zoek — Gehandicapten Parkeer Plaats Zoek (v1.0)
...
class GpkApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'GPP Zoek — Gehandicapten Parkeer Plaats',
  ...
  void _openOver() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: const Text('Over GPP Zoek'),
...
class _PlekSheetState extends State<PlekSheet> {
  ...
  Future<void> _open(String url) async {
    ...
    // Niet vertrouwen op canLaunchUrl (geeft soms onterecht false); gewoon
    // proberen te openen, met een nette terugval en melding bij falen.
    try {
...
// ---------------------------------------------------------------------------
// Helppagina — uitleg over het gebruik van de app.
// ---------------------------------------------------------------------------
class HelpPagina extends StatelessWidget {
  const HelpPagina({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: kDiep,
        foregroundColor: Colors.white,
        title: const Text('Help & uitleg'),
        elevation: 0,
      ),
      body: ListView(
        padding: const EdgeInsets.fromLTRB(20, 18, 20, 40),
        children: const [
          _HelpKop('Wat doet deze app?'),
          _HelpTekst(
              'GPP Zoek toont gehandicaptenparkeerplaatsen in Nederland op de '
              'kaart, en laat per plek de parkeerregels van de gemeente zien.'),

          _HelpKop('Een plaats zoeken'),
          _HelpStap(1, 'Typ een plaatsnaam in het zoekvak bovenaan en druk op '
              'Zoek (of Enter).'),
          _HelpStap(2, 'De kaart springt naar die plaats en toont de '
              'gehandicaptenplekken die daar bekend zijn.'),

          _HelpKop('De knoppen onderaan'),
          _HelpItem('Download nieuwste',
              'Haalt de plekken op voor het gebied dat je nu op de kaart ziet. '
              'Wat er stond wordt vervangen door de nieuwste gegevens.'),
          _HelpItem('Bijwerken',
              'Vult het huidige beeld aan met plekken, zonder te wissen. Handig '
              'nadat je de kaart een stukje hebt verschoven.'),
          _HelpTint(
              'Tip: zoom eerst in op de wijk of stad die je wilt bekijken en '
              'druk dan op Download nieuwste. Per keer worden maximaal 800 '
              'plekken opgehaald voor het zichtbare gebied.'),

          _HelpKop('Details en regels bekijken'),
          _HelpStap(1, 'Tik op een blauwe markering op de kaart.'),
          _HelpStap(2, 'Onderin schuift een venster open met de details van de '
              'plek (aantal plekken, betaald, max. duur, enzovoort).'),
          _HelpStap(3, 'Daaronder staan de parkeerregels van de gemeente, live '
              'opgehaald van gpkwijs.nl, met links naar de volledige regels en '
              'de gemeentesite.'),

          _HelpKop('Route naar een plek'),
          _HelpTekst(
              'Tik in het detailvenster op "Routebeschrijving". Google Maps '
              '(of je kaart-app) opent met een route naar die parkeerplaats.'),

          _HelpKop('Waarom zie ik niet overal plekken?'),
          _HelpTekst(
              'De plekken komen uit OpenStreetMap, een open kaart die door '
              'vrijwilligers wordt bijgehouden. Niet elke gehandicaptenplek is '
              'al ingetekend: grote steden zijn vaak compleet, kleinere plaatsen '
              'soms nog niet. Een leeg gebied betekent meestal dat de plekken daar '
              'nog niet in OpenStreetMap staan — niet dat de app iets mist. '
              'Ontbrekende plekken kun je zelf toevoegen op openstreetmap.org; ze '
              'verschijnen dan later ook hier.'),

          _HelpKop('Internet nodig'),
          _HelpTekst(
              'De app haalt kaarten, plekken en regels live op. Er is dus een '
          ...
          SizedBox(height: 20),
          Center(
            child: Text('GPP Zoek v1.0 · door Michael van der Meijden',
                style: TextStyle(color: kDemping, fontSize: 12)),
          ),
        ],
      ),
    );
...
class _HelpKop extends StatelessWidget {
  final String tekst;
  const _HelpKop(this.tekst);
  @override
  Widget build(BuildContext context) => Padding(
        padding: const EdgeInsets.only(top: 22, bottom: 8),
        child: Text(tekst,
            style: const TextStyle(
                fontSize: 16, fontWeight: FontWeight.w700, color: kInkt)),
      );
}

class _HelpTekst extends StatelessWidget {
  final String tekst;
  const _HelpTekst(this.tekst);
  @override
  Widget build(BuildContext context) => Text(tekst,
      style: const TextStyle(fontSize: 14, height: 1.5, color: kInkt));
}

class _HelpStap extends StatelessWidget {
  final int n;
  final String tekst;
  const _HelpStap(this.n, this.tekst);
  @override
  Widget build(BuildContext context) => Padding(
          ...
          children: [
            Container(
              ...
              decoration: const BoxDecoration(
                  color: kLucht, shape: BoxShape.circle),
              alignment: Alignment.center,
              child: Text('$n',
                  style: const TextStyle(
                      color: kBlauw,
...
class _HelpItem extends StatelessWidget {
  final String titel, tekst;
  const _HelpItem(this.titel, this.tekst);
  @override
  Widget build(BuildContext context) => Padding(
        padding: const EdgeInsets.only(bottom: 12),
        child: Column(
          ...
          children: [
            Text(titel,
                style: const TextStyle(
                    fontSize: 14, fontWeight: FontWeight.w700, color: kBlauw)),
            const SizedBox(height: 2),
            Text(tekst,
                style: const TextStyle(fontSize: 14, height: 1.45, color: kInkt)),
          ],
        ),
      );
}

class _HelpTint extends StatelessWidget {
  final String tekst;
  const _HelpTint(this.tekst);
  @override
  Widget build(BuildContext context) => Container(
        ...
        child: Text(tekst,
            style: const TextStyle(fontSize: 13, height: 1.45, color: kInkt)),
      );
}
